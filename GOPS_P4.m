%% Pressure determination using Grt-Opx-Plag-Qtz geobarometer
% Last update: 20.06.2016
clc; clear; close all; format compact; format short;

fprintf('_________________GRT-OPX-PlAG-QTZ GEOBAROMETRY__________________\n\n');
...
fprintf (' then M1-and M2- XMg for Orthopyroxene(6-O) in single line.\n');
fprintf (' # Enter each dataset in a new line. \n\n');

fprintf (' [CAUTION: FOR OPX- Entries are XMg-M1 site and XMg-M2 site]\n');
...
XMg1_Opx = data(:,7); XMg2_Opx = data(:,8);
...
for i=1:dataset
    ...
    % activitiy of ANORTHITE; after Newton et al. 1980
    ...
    % activitiy of ENSTATITE; after Wood & Banno 1973
    a_En = XMg1_Opx(i) * XMg2_Opx(i);
     
    % Kd calculation for An + En = 1/3 Grs + 2/3 Prp + Qtz
    Kd = (a_Grs*a_Prp*a_Prp)/(a_An*a_En); ln_Kd = log(Kd);
    
    %% Newton & Perkins (1982,Am Min) formulation
    P_NP(i) = 3.944 + 0.01307*T_Kel + 0.0035038*T_Kel*ln_Kd;
    
    %% Eckert et al. (1991,Am Min) formulation
    P_Eck(i) = 3.47 + 0.01307*T_Kel + 0.0035038*T_Kel*ln_Kd;
    
    %% Bhattarcharya et al.1991,JP Garnet and Plagioclase activity model 
    %  using Eckert et al. formulation
    %  Plag activity is originally after Newton 1983, AJS
    %  Opx activity is after Wood and Banno 1973
    ...
    % activitiy of ANORTHITE; after Newton et al. 1983
    XAn = Ca_Pl(i)/(Ca_Pl(i)+ Na_Pl(i));
    XAb = Na_Pl(i)/(Ca_Pl(i)+ Na_Pl(i));
    ...
    P_Eck_B(i) = 3.47 + 0.01307*T_Kel + 0.0035038*T_Kel*ln_Kd;
    
    %% Moecher et al.1988,CMP log(10) K isopleths
    %  using Eckert et al. formulation
    %  Garnet activity model after Anovitz and Essene 1987
    %  Plag activity is after Newton 1983, AJS
    %  Opx activity is after Wood and Banno 1973
    
    % activities of GARNET end-members; after Anovitz & Essene 1987
    % XFe_Grt;XMg_Grt;XCa_Grt -same as calculated for Bhattacharya et al.
    ...
    Kd    = (a_Grs*a_Prp*a_Prp)/(a_An*a_En); ln_Kd = log(Kd);  
    % Eckert et al. 1991 formulation
    P_Eck_M(i) = 3.47 + 0.01307*T_Kel + 0.0035038*T_Kel*ln_Kd;
...
results = [SlNo,P_NP,P_Eck,P_Eck_B,P_Eck_M];
...
% printing on screen
fprintf ('\nSummary of Results:\n\n');
names = [' S.No.',' NP-82',' Eck-91',' Eck_Bh',' Eck_Mo'];
...
fprintf('------------------------------------------------------------------------\n');
fprintf ('Eck_Bh: Grt_activity-Bhattacharya et al. 1991; Plag_acitivty-Newton 1983 \n');
fprintf ('Eck_Mo: Grt_activity-Moecher et al. 1988; Plag_activity-Newton 1983 \n');
fprintf ('Opx_activity: Wood and Banno 1973 in all calculations \n');
fprintf ('\n\n');
