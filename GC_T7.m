...
%% Different Geothermometric expressions
    ...
    % 4 -- Krogh 1988, CMP
    % 5 -- Powell 1985, JMG
    % 6 -- Ellis & Green 1979, CMP
    % 7 -- Ganguly 1996, -- only plotting results
...
results = [SlNo,round(T_Nak),round(T_Ravna),round(T_Ai),round(T_Krogh),...
    round(T_Po),round(T_EG),round(T_Gang)];
...
res_mat(4,:) = [Krogh_max,Krogh_min,Krogh_avg];
res_mat(5,:) = [Po_max,Po_min,Po_avg];
res_mat(6,:) = [EG_max,EG_min,EG_avg];
res_mat(7,:) = [Gan_max,Gan_min,Gan_avg];

mean_T = mean([Nak_avg,Ravna_avg,Ai_avg,Krogh_avg,Po_avg,EG_avg,Gan_avg]);

% printing on screen
fprintf ('\nSummary of Results:\n\n');

names = ['S.No.',' Nak-09',' Rav-00','  Ai-94',' Kro-88',' Pow-85',...
    ' E&G-79',' Gan-96'];
...
%% Plotting the result
plot(1:dataset,T_Nak,'p-',1:dataset,T_Ravna,'.-',1:dataset,T_Ai,'s-',1:dataset,T_Krogh,'x-',...
    1:dataset,T_Po,'*-',1:dataset,T_EG,'d-',1:dataset,T_Gang,'o-');
axis tight;
legend('Nak-09','Rav-00','Ai-94','Kro-88','Pow-85','E&G-79','Gan-96');
